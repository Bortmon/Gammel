...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  String? _detailOldPriceString; String? _detailDiscountLabel; String? _detailPromotionDescription; String? _detailPricePerUnitString; String? _detailPriceUnit; String? _detailPricePerUnitLabel;
  ...
  @override void initState() { super.initState(); _detailImageUrl = widget.product.imageUrl; _detailPriceString = widget.product.priceString; _detailOldPriceString = widget.product.oldPriceString; _detailDiscountLabel = widget.product.discountLabel; _detailPromotionDescription = widget.product.promotionDescription; _detailPricePerUnitString = widget.product.pricePerUnitString; _detailPriceUnit = widget.product.priceUnit; _detailPricePerUnitLabel = widget.product.pricePerUnitLabel; _fetchProductDetails(); _fetchSpecificStoreStocks(); }
  Future<void> _fetchProductDetails() async { setState(() { _isLoadingDetails = true; _description = null; _specifications = null; _detailsError = null; }); if (widget.product.productUrl == null) { setState(() { _detailsError = "URL?"; _isLoadingDetails = false; }); return; } final url = Uri.parse(widget.product.productUrl!); print('[Parser Details] Fetching: $url'); try { final response = await http.get(url, headers: {'User-Agent': _userAgent}); if (!mounted) return; if (response.statusCode == 200) { final document = parse(response.body); String? pDesc; String? pSpecs; String? fImgUrl = _detailImageUrl; String? newPrice; String? newOldPrice; String? newDiscount; String? newPromoDesc; String? newPricePerUnit; String? newPriceUnit; String? newPricePerUnitLabel; final infoEl = document.querySelector('#product-info-content'); if (infoEl != null) { String short = infoEl.querySelectorAll('div.product-info-short ul li').map((li) => '• ${li.text.trim()}').join('\n'); final descEl = infoEl.querySelector('div.description div[itemprop="description"] p') ?? infoEl.querySelector('div.description p'); String main = descEl?.text.trim() ?? ''; List<String> parts = []; if (short.isNotEmpty) parts.add(short); if (main.isNotEmpty) parts.add(main); pDesc = parts.join('\n\n').trim(); if (pDesc.isEmpty) pDesc = null; } final specsEl = document.querySelector('#product-specs'); if (specsEl != null) { final List<String> lines = []; final tables = specsEl.querySelectorAll('table.fancy-table'); if (tables.isNotEmpty) { for (var t in tables) { final h = t.querySelector('thead tr.group-name th strong'); if (h != null) { if (lines.isNotEmpty) lines.add(''); lines.add('${h.text.trim()}:'); } final rows = t.querySelectorAll('tbody tr'); for (var r in rows) { final kE = r.querySelector('th.attrib'); final vE = r.querySelector('td.value .feature-value'); if (kE != null && vE != null) { final k = kE.text.trim(); final v = vE.text.trim(); if (k.isNotEmpty) lines.add('  $k: $v'); } } } pSpecs = lines.join('\n').trim(); if (pSpecs.isEmpty) pSpecs = 'Specs leeg.'; } else { pSpecs = 'Geen specs tabel.'; } } else { pSpecs = 'Specs niet gevonden.'; } final imgEl = document.querySelector('img.product-main-image'); if (imgEl != null) { String? dS = imgEl.attributes['data-src']; String? s = imgEl.attributes['src']; String? tmp = dS ?? s; if (tmp != null && tmp.contains('/placeholders/')) { String? alt = (tmp == dS) ? s : dS; if (alt != null && !alt.contains('/placeholders/')) { tmp = alt; } else { tmp = null; } } if (tmp != null && !tmp.startsWith('http')) tmp = null; if (tmp != null) fImgUrl = tmp; } else { final metaImg = document.querySelector('meta[itemprop="image"]'); String? mUrl = metaImg?.attributes['content']; if (mUrl != null && mUrl.startsWith('http')) fImgUrl = mUrl; } newPrice = document.querySelector('meta[itemprop="price"]')?.attributes['content']?.trim(); newOldPrice = document.querySelector('.product-price-base .before-price')?.text.trim() ?? document.querySelector('.pdp-price__retail .price-amount')?.text.trim() ?? document.querySelector('.price-suggested .price-amount')?.text.trim() ?? document.querySelector('span[data-price-type="oldPrice"] .price')?.text.trim(); if (newPrice == null || newPrice.isEmpty) { final priceElement = document.querySelector('.price-sales-standard .price-amount') ?? document.querySelector('.pdp-price__integer'); final decimalElement = document.querySelector('.pdp-price__fractional'); if (priceElement != null && decimalElement == null) { newPrice = priceElement.text.trim().replaceAll(RegExp(r'[^\d,.]'), '').replaceFirst(',', '.'); } else if (priceElement != null && decimalElement != null) { final intP = priceElement.text.trim(); final decP = decimalElement.text.trim(); if (intP.isNotEmpty && decP.isNotEmpty) { newPrice = '$intP.$decP'; } } } if (newOldPrice != null) { newOldPrice = newOldPrice.replaceAll(RegExp(r'[^\d,.]'), '').replaceFirst(',', '.'); if (newOldPrice.isEmpty) newOldPrice = null; } if (newOldPrice != null && newPrice == null) { newPrice = _detailPriceString; } newPriceUnit = document.querySelector('.pdp-price__unit')?.text.trim() ?? document.querySelector('.product-tile-price-unit')?.text.trim(); if (newPriceUnit != null && newPriceUnit.isEmpty) newPriceUnit = null; final pricePerUnitContainer = document.querySelector('.product-price-per-unit'); if (pricePerUnitContainer != null) { final pricePerUnitElement = pricePerUnitContainer.querySelector('span:last-child'); if (pricePerUnitElement != null) { newPricePerUnit = pricePerUnitElement.text.trim().replaceAll(RegExp(r'[^\d,.]'), '').replaceFirst(',', '.'); if (newPricePerUnit.isEmpty || newPricePerUnit == newPrice) newPricePerUnit = null; } final pricePerUnitLabelElement = pricePerUnitContainer.querySelector('span:first-child'); if (pricePerUnitLabelElement != null) { newPricePerUnitLabel = pricePerUnitLabelElement.text.trim(); if (newPricePerUnitLabel.isEmpty) newPricePerUnitLabel = null; } } final promoInfoLabel = document.querySelector('.promotion-info-label div div'); if (promoInfoLabel != null) { newDiscount = promoInfoLabel.text.trim(); } else { newDiscount = document.querySelector('.product-labels .label-item')?.text.trim() ?? document.querySelector('.sticker-action')?.text.trim() ?? document.querySelector('.product-badge .badge-text')?.text.trim(); } if (newDiscount != null && newDiscount.isEmpty) newDiscount = null; if (newDiscount == null && newOldPrice != null && newPrice != null && newOldPrice != newPrice) { newDiscount = "Actie"; } final promoDescElement = document.querySelector('dd.promotion-info-description'); if (promoDescElement != null) { newPromoDesc = promoDescElement.text.trim().replaceAll(RegExp(r'Bekijk alle producten.*$', multiLine: true), '').replaceAll(RegExp(r'\s+'), ' ').trim(); if (newPromoDesc.isEmpty) newPromoDesc = null; } if(mounted){setState((){_description=pDesc;_specifications=pSpecs;_detailImageUrl=fImgUrl;if(newPrice!=null&&newPrice.isNotEmpty)_detailPriceString=newPrice;_detailOldPriceString=newOldPrice;_detailDiscountLabel=newDiscount;_detailPromotionDescription=newPromoDesc;_detailPricePerUnitString = newPricePerUnit; _detailPriceUnit = newPriceUnit; _detailPricePerUnitLabel = newPricePerUnitLabel; _detailsError=(_description==null&&_specifications==null)?'Kon details niet laden.':null;});}} else { if (mounted) setState(() { _detailsError = 'Fout details: ${response.statusCode}'; }); } } catch (e) { print('[Parser Details] Error: $e'); if (mounted) setState(() { _detailsError = 'Fout verwerken: $e'; }); } finally { if (mounted) { setState(() { _isLoadingDetails = false; }); } } }
  Future<void> _fetchSpecificStoreStocks() async { setState(() { _isLoadingStock = true; _stockError = null; _storeStocks = {}; }); String pId = widget.product.articleCode; if (pId == 'Code niet gevonden') { setState(() { _stockError = "Code?"; _isLoadingStock = false; }); return; } else { try { pId = int.parse(pId).toString(); } catch (e) {} } Map<String, int?> stocks = {}; String err = ''; final gEntries = _targetStores.entries.where((e) => e.key.startsWith('Gamma')); final kEntries = _targetStores.entries.where((e) => e.key.startsWith('Karwei')); final gParam = gEntries.map((e) => 'Stock-${e.value}-${pId}').join(','); final kParam = kEntries.map((e) => 'Stock-${e.value}-${pId}').join(','); List<Future<void>> calls = []; if (gParam.isNotEmpty) { final url = Uri.parse('$gammaStockApiBase?uids=$gParam'); final h = {'User-Agent': _userAgent, 'Origin':'https://www.gamma.nl', 'Referer':'https://www.gamma.nl/', 'Cookie':'$gammaCookieName=$gammaCookieValueHaarlem'}; calls.add( http.get(url, headers:h).then((r) { if(r.statusCode==200){try{final d=jsonDecode(r.body) as List;for(var e in gEntries){final u='Stock-${e.value}-${pId}';var s=d.firstWhere((i)=>i is Map&&i['uid']==u, orElse:()=>null);if(s!=null){final q=s['quantity'];stocks[e.key]=(q is int)?q:((q is String)?int.tryParse(q):null);}else{stocks[e.key]=null;}}}catch(e){err+=' G P.';print("G Stock Parse Err: $e");}}else{err+=' G(${r.statusCode}).';}}).catchError((e){err+=' G N.';print("G Stock Net Err: $e");}) ); } if (kParam.isNotEmpty) { final url = Uri.parse('$karweiStockApiBase?uids=$kParam'); final h = {'User-Agent': _userAgent, 'Origin':'https://www.karwei.nl', 'Referer':'https://www.karwei.nl/'}; calls.add( http.get(url, headers:h).then((r) { if(r.statusCode==200){try{final d=jsonDecode(r.body) as List;for(var e in kEntries){final u='Stock-${e.value}-${pId}';var s=d.firstWhere((i)=>i is Map&&i['uid']==u, orElse:()=>null);if(s!=null){final q=s['quantity'];stocks[e.key]=(q is int)?q:((q is String)?int.tryParse(q):null);}else{stocks[e.key]=null;}}}catch(e){err+=' K P.';print("K Stock Parse Err: $e");}}else{err+=' K(${r.statusCode}).';}}).catchError((e){err+=' K N.';print("K Stock Net Err: $e");}) ); } await Future.wait(calls); if (mounted) { setState(() { _storeStocks = stocks; _stockError = err.isEmpty ? null : err.trim(); _isLoadingStock = false; }); } }
          ...
          Row( crossAxisAlignment: CrossAxisAlignment.center, children: [
              Expanded( child: Column( crossAxisAlignment: CrossAxisAlignment.start, children: [
                  if (_isLoadingDetails && _detailPriceString == null) Text("Prijs laden...", style: txt.titleLarge?.copyWith(color: Colors.grey))
                     ...
                     RichText( text: TextSpan( style: txt.headlineSmall?.copyWith(color: clr.onSurface), children: [
                           ...
                           TextSpan( text: '€ $_detailPriceString', style: TextStyle( color: clr.secondary, fontWeight: FontWeight.bold,),),
                           if (_detailPriceUnit != null) TextSpan( text: ' ${_detailPriceUnit}', style: txt.bodySmall?.copyWith(color: clr.onSurfaceVariant) )
                         ],),)
                  else Text('Prijs?', style: txt.bodyMedium?.copyWith(fontStyle: FontStyle.italic, color: Colors.grey),),
                  if (_detailPricePerUnitString != null) Padding( padding: const EdgeInsets.only(top: 4.0), child: Text( '€ $_detailPricePerUnitString ${(_detailPricePerUnitLabel ?? "per eenheid").toLowerCase()}', style: txt.bodyMedium?.copyWith(fontWeight: FontWeight.w500), ), ),
                ],),
              ),
              const SizedBox(width: 12),
              // --- FIX: Gebruik correcte variabele naam ---
              if (_detailDiscountLabel != null)
                 Tooltip(
                   message: isDiscountTappable ? "Bekijk details" : "", // Gebruik isDiscountTappable
                   child: Material( color: Colors.transparent, child: InkWell(
                       onTap: isDiscountTappable ? () => _showPromotionDetails(context) : null, // Gebruik isDiscountTappable
                       borderRadius: BorderRadius.circular(6.0),
                       child: Container( padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration( color: isDarkMode ? Colors.orange[700] : clr.primary, borderRadius: BorderRadius.circular(6.0), ),
                         child: Row( mainAxisSize: MainAxisSize.min, children: [
                             Text( _detailDiscountLabel!, style: txt.labelMedium?.copyWith( color: isDarkMode ? Colors.white : clr.onPrimary, fontWeight: FontWeight.bold, ), overflow: TextOverflow.ellipsis, ),
                             if(isDiscountTappable) Padding( padding: const EdgeInsets.only(left: 4.0), child: Icon(Icons.info_outline, size: txt.labelMedium?.fontSize ?? 14, color: (isDarkMode ? Colors.white : clr.onPrimary).withOpacity(0.7)), ) // Gebruik isDiscountTappable
                           ],),
                       ),),),
                 ),
              // --- EINDE FIX ---
  ...
  Widget _buildDetailsSection(BuildContext context, TextTheme textTheme) {
    final clr = Theme.of(context).colorScheme;
     ...
     } else if (_detailsError != null && _description == null && _specifications == null) {
