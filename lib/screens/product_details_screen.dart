...
import 'scanner_screen.dart'; // Importeer scanner screen
...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  String? _detailOldPriceString; String? _detailDiscountLabel; String? _detailPromotionDescription; bool _isLoadingDetails = true;
  ...
  @override
  void initState() { super.initState(); _detailImageUrl = widget.product.imageUrl; _detailPriceString = widget.product.priceString; _detailOldPriceString = widget.product.oldPriceString; _detailDiscountLabel = widget.product.discountLabel; _detailPromotionDescription = widget.product.promotionDescription; _fetchProductDetails(); _fetchSpecificStoreStocks(); }

  Future<void> _fetchProductDetails() async {
    setState(() { _isLoadingDetails = true; _description = null; _specifications = null; _detailsError = null; }); if (widget.product.productUrl == null) { setState(() { _detailsError = "URL?"; _isLoadingDetails = false; }); return; }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if(mounted){setState((){_description=pDesc;_specifications=pSpecs;_detailImageUrl=fImgUrl;if(newPrice!=null&&newPrice.isNotEmpty)_detailPriceString=newPrice;_detailOldPriceString=newOldPrice;_detailDiscountLabel=newDiscount;_detailPromotionDescription=newPromoDesc;_detailsError=(_description==null&&_specifications==null)?'Kon details niet laden.':null;});}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold( // <--- Start Scaffold
      ...
      body: SingleChildScrollView( padding: const EdgeInsets.all(16.0), child: Column( crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row( crossAxisAlignment: CrossAxisAlignment.end, children: [
              if (_isLoadingDetails && _detailPriceString == null) Text("Prijs laden...", style: txt.titleLarge?.copyWith(color: Colors.grey))
                 ...
                 RichText( text: TextSpan( style: txt.headlineSmall, children: [
              ...
              // --- TOOLTIP TOEGEVOEGD ROND GESTUREDETECTOR ---
              if (_detailDiscountLabel != null)
                 Flexible(
                   child: Tooltip( // Wrap met Tooltip
                     message: isDiscountChipTappable ? "Bekijk details" : "", // Toon alleen tooltip als klikbaar
                     child: GestureDetector(
                       onTap: isDiscountChipTappable ? () => _showPromotionDetails(context) : null,
                       child: Chip(
                          label: Row( mainAxisSize: MainAxisSize.min, children: [
                              Text(_detailDiscountLabel!, overflow: TextOverflow.ellipsis),
                              if(isDiscountChipTappable) Padding( padding: const EdgeInsets.only(left: 4.0), child: Icon(Icons.info_outline, size: txt.labelSmall?.fontSize ?? 12, color: isDarkMode ? Colors.black54 : clr.onErrorContainer.withOpacity(0.7)), )
                           ],),
                          labelStyle: txt.labelSmall?.copyWith(color: isDarkMode ? Colors.black : clr.onErrorContainer, fontWeight: isDarkMode ? FontWeight.bold : FontWeight.normal,),
                          backgroundColor: isDarkMode ? Colors.orange[700] : clr.errorContainer,
                          padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                          materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                          visualDensity: VisualDensity.compact,
                          shape: RoundedRectangleBorder( borderRadius: BorderRadius.circular(4.0), side: BorderSide.none, ),
                       ),
                     ),
                   ),
                 ),
    ...
    // --- FIX: Sluithaakje voor SingleChildScrollView ---
    ),
