...
class _HomePageState extends State<HomePage> {
  ...
  List<Product> _parseProducts(dom.Document document) {
    final products = <Product>[]; final productElements = document.querySelectorAll('article.js-product-tile');
    for (final element in productElements) { String? imageUrl; String? priceString; try { String title = element.querySelector('div.product-tile-name a')?.text.trim() ?? element.querySelector('a.click-mask')?.attributes['title']?.trim() ?? 'Titel?'; String? productUrl = element.querySelector('a.click-mask')?.attributes['href']; if (productUrl != null && !productUrl.startsWith('http')) { if (!productUrl.startsWith('/')) { productUrl = '/$productUrl'; } productUrl = 'https://www.gamma.nl$productUrl'; } String? articleCode = element.attributes['data-objectid']?.trim(); if (articleCode != null && articleCode.isNotEmpty && articleCode.length > 1) { if (int.tryParse(articleCode.substring(0, 1)) == null) { articleCode = articleCode.substring(1); } } String? eanCode = element.attributes['data-ean']?.trim(); if (articleCode == null || articleCode.isEmpty) { articleCode = eanCode ?? 'Code?'; eanCode = null; } else if (eanCode == null || eanCode.isEmpty) { eanCode = null; } final imageContainer = element.querySelector('div.product-tile-image'); if (imageContainer != null) { final imageElement = imageContainer.querySelector('img:not(.sticker)'); if (imageElement != null) { imageUrl = imageElement.attributes['data-src'] ?? imageElement.attributes['src']; } else { final fallbackImageElement = imageContainer.querySelector('img'); imageUrl = fallbackImageElement?.attributes['data-src'] ?? fallbackImageElement?.attributes['src']; } if (imageUrl != null && !imageUrl.startsWith('http')) imageUrl = null; } final priceIntElement = element.querySelector('div.product-tile-price-current'); final priceDecElement = element.querySelector('span.product-tile-price-decimal'); if (priceIntElement != null && priceDecElement != null) { String intPart = priceIntElement.text.trim().replaceAll('.', ''); String decPart = priceDecElement.text.trim(); if (intPart.isNotEmpty && decPart.isNotEmpty) { priceString = '$intPart.$decPart'; } } if (priceString == null) { String? dataPrice = element.attributes['data-price']?.trim(); if (dataPrice != null) priceString = dataPrice; } if (title != 'Titel?' && articleCode != 'Code?') { products.add(Product( title: title, articleCode: articleCode, eanCode: eanCode, imageUrl: imageUrl, productUrl: productUrl, priceString: priceString )); } } catch (e) { print("[Parser Results] Error: $e"); } } return products;
  }
  ...
  void _navigateToScheduleScreen(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(builder: (context) => ScheduleScreen(
          ...
          userName: widget.userName, // <-- Geef naam door
  ...
  Widget _buildResultsArea() {
    final txt = Theme.of(context).textTheme; final clr = Theme.of(context).colorScheme;
